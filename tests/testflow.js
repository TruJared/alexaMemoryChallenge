...
// console.log('================================================================================');
console.log(
  'Running testflow on ' + SourceCodeFile + ' using dialog sequence file ',
  MyDialog
);
console.log();
...
let context = {
  // lambda functions may finish by calling context.succeed OR the callback function passed as the third argument
  succeed: function(data) {
    // console.log('context.succeed:\n' + JSON.stringify(data,null, 2));

    if (
      data.response.shouldEndSession ||
      typeof data.response.shouldEndSession === 'undefined'
    ) {
      sa = {};
    } else {
    ...
    let textToSay =
      data.response.outputSpeech.ssml || data.response.outputSpeech.text;
    ...
    if (
      ...
      data.response.reprompt.outputSpeech &&
      data.response.reprompt.outputSpeech.ssml
    ) {
    ...
    if (options.cards) {
      if (data.response.card) {
        ...
        console.log();
        linePadding =
          cardWidth < cardTitle.length ? 0 : cardWidth - cardTitle.length;
        console.log(
          '\x1b[44m\x1b[4m\x1b[37m%s\x1b[0m\x1b[0m ',
          cardTitle + ' '.repeat(linePadding)
        );
        ...
        for (let i = 0; i < cardContentLines.length; i++) {
          ...
          console.log(
            ...
            cardContentLines[i] + ' '.repeat(linePadding)
          );
        }
    ...
    if (typeof options.attributes == 'boolean') {
      ...
      // you can define an attribute to display by setting options.attribute to a string, such as 'STATE'
      ...
      if (!printAttribute) {
        ...
        printAttributeObject = JSON.parse(
          '{"' + printAttributeName + '":"' + printAttribute + '"}'
        );
      }
...
fs.readFile(MyDialog, function(err, data) {
  // open dialog sequence file and read Intents
  if (err) {
...
function runSingleTest(myLineArray, currentLine, sa) {
  ...
  if (tokenArray[0].replace('\r', '') == '?') {
    // pause and prompt the user to confirm
    prompt = true;
    // console.log(' ----------------- > prompt');
    tokenArray.shift(); // removes first item
  }

  if (tokenArray[0].replace('\r', '') == 'end') {
    // quit and skip remaining lines
    process.exit(1);
  }
  ...
  if (requestType == 'LaunchRequest') {
    ...
    console.log(
      '%s \x1b[33m\x1b[1m%s\x1b[0m \x1b[2m%s\x1b[0m',
      currentLine + 1,
      Intent,
      sdkState
    );
...
function processArray(arr, cb) {
  // process multiple slots as array
  const UTCnow = new Date().toISOString();

  if (arr.length > 0) {
    let equalsPosition = arr[0].indexOf('=');
    slotname = arr[0].substr(0, equalsPosition);
    slotvalue = decodeURI(arr[0].substr(equalsPosition + 1, 300)).replace(
      '\r',
      ''
    );
...
function prepareTestRequest(sa, newSession, request) {
  const eventJSON = {
    context: {
      ...
      System: {
        ...
        user: {
          userId:
            'amzn1.ask.account.AG4BEMGBJIJYRAGKP5YLYGJDISRIQVPWOEABD3OQW66MTPOF4JRHLAEETH5TBIOT652I3KUYZSWA5MAZ33GUTIBLEMUQ4YQBPKXVG4YSFHLQL27UEC6YQDXTEYH5MD4NMK4M7UJ4FWTRPKTSII4R733EX3TVC3UKLKOBMHXXM5CHOX2TDUV2WPF6NCWF5KLJETNGQ5YC7UID' +
            userIdLast3
        },
        device: {
          deviceId:
  ...
  if (SourceLang === 'javascript') {
    ...
    let sourcePath = SourceCodeFile.substring(
      0,
      SourceCodeFile.lastIndexOf('/')
    );
    let sourceFileName = SourceCodeFile.substring(
      SourceCodeFile.lastIndexOf('/') + 1,
      SourceCodeFile.length - 3
    );
    ...
    const pycodeArray = [
      ...
      "response = lambda_handler(json.loads('" +
...
function promptForSlot(prompt, slotname, slotvalue, callback) {
  ...
  if (prompt) {
    process.stdout.write(
      '  \x1b[34m' + slotname + ' \x1b[0m\x1b[32m [' + slotvalue + ']\x1b[0m: '
    );
    ...
    process.stdin.once('data', function(data) {
      ...
      if (answer == '') {
        if (slotvalue == '') {
          ...
          console.error(
            'Error: No default slot value defined, user must type a slot value.'
          );
          process.exit();
        } else {
          answer = slotvalue;
...
function cleanArray(myArray) {
  ...
  for (let i = 0; i < myArray.length; i++) {
    if (
      myArray[i] != '' &&
      myArray[i].substring(0, 1) != '#' &&
      myArray[i].substring(0, 2) != '//'
    ) {
...
function addTime(startDateTime, delta) {
  ...
  if (timeUnit == 'M' || timeUnit == 'H' || timeUnit == 'D') {
    ...
    console.log(
      'invalid time offset.  An example format for 6 hours back is : -6H'
    );
  }
...
function buildSlotObj(slotname, val) {
  ...
  if (!val || val === '') {
    ...
    if (slashPos <= 0) {
      ...
      ERstatus = 'SimpleSlot';
      obj =
        '"' + slotname + '": {"name":"' + slotname + '","value":"' + val + '"}';
    } else {
      // console.log('***** ER slot');
